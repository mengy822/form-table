...
import type { tableColumnItem } from '../../Table/props'
export default {
  name: 'MyTableColumn',
  components: {
    ElTableColumn,
  ...
  setup() {
    ...
    function renderColumnList(columns: tableColumnItem[] = []) {
      const $self = this
      return columns.map((column) => {
        if (column.hidden && !column.visible) {
          return undefined
        }
        let slot: { header?: (scope: any) => Function; default?: (scope: any) => Function } = {} // 插槽
        if (column.header) {
        ...
        slot.default = (scope) => {
          ...
          if (Array.isArray(column.list) && column.list.length) {
            ...
            return generateColumnSlot.call($self, column.slot, {
              row, prop: column.prop, index: $index, fun: column.fun,

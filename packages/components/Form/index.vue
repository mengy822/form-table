  ...
  <div v-show="showSearch" class="mb-[20px]">
    <el-card shadow="hover">
      <el-config-provider :locale="language">
        <div class="searchList form form-plus-main" ref="formPlusMain" :class="{ hide: !showSearch, show: showSearch }">
          <div
          ...
          >
            <el-form ref="formRef" v-show="showSearch" :inline="true" :model="dynamicComputedMap" :label-width="labelWidth">
              <slot
              ...
              >
                <el-form-item
                  ...
                  :rules="
                    ...
                    (item.dynamicRequired && item.dynamicRequired(dynamicComputedMap) && rules[item.prop])
                      ? rules[item.prop]
                      : []
                  "
                  v-if="typeof item.showFun === 'undefined' || (item.showFun && item.showFun(dynamicComputedMap))"
                >
                  ...
                  <template #default>
                    <slot :name="item.prop" :prop="item.prop" :data="dynamicComputedMap">
                      <Input :data="item as inputInnerType" v-if="item.type === 'input'" v-model="dynamicComputedMap[item.prop]"></Input>
                      <MyDate :data="item as dateInnerType" v-if="item.type === 'date'" v-model="dynamicComputedMap[item.prop]"></MyDate>

                      <Select :data="item as selectInnerType" v-if="item.type === 'select'" v-model="dynamicComputedMap[item.prop]"></Select>
                      <Switch :data="item as switchInnerType" v-if="item.type === 'switch'" v-model="dynamicComputedMap[item.prop]"></Switch>
                      <CheckBox :data="item as checkboxInnerType" v-if="item.type === 'checkbox'" v-model="dynamicComputedMap[item.prop]"></CheckBox>
                      <Radio :data="item as radioInnerType" v-if="item.type === 'radio'" v-model="dynamicComputedMap[item.prop]"></Radio>
                    </slot>
                  </template>
                </el-form-item>
              </slot>
              ...
              <el-form-item v-if="rowIndex === 0" class="buttons">
                <div style="display: flex" :gutter="gutter" ref="buttons">
                  <div v-for="item in searchButtonFinal" :key="JSON.stringify(item)" style="display: flex" class="my-form-buttons">
                    <el-button
  ...
  nextTick(() => {
    ...
    for (let i = 0; i < inputWidthKeys.length; i++) {
      ...
      if (sum - inputWidth < 0) {
        ...
        oneRow.push(search.find((item) => item.prop === inputWidthKey.replace('Ref', '')) as inputInnerType | dateInnerType | selectInnerType);
        sum -= inputWidth;
      }
    }
    if (oneRow.length > 0) {

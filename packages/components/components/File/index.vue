<template>
  ...
  >
    ...
    <template #tip>
      ...
      <div class="el-upload__tip">
        <span v-if="dataFinal.hasTemplate">上传文件 <el-button type="text" @click="downloadFun(dataFinal.hasTemplate)">模板文件下载</el-button></span>
      </div>
    </template>
  </el-upload>
</template>
...
import { ElMessage, UploadProps, UploadRequestOptions, UploadUserFile } from 'element-plus';
...
const handleOnRemove = (file, uploadFiles) => {
...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
  ...
  if (dataFinal.value.accept && !dataFinal.value.accept.split(',').find((item) => item === (type as string))) {
    emits('fileTypeError');
    ...
    return false;
  } else if (rawFile.size / 1024 / 1024 > dataFinal.value.limitSize) {
    emits('fileSizeError');
...
const handleRequest = (options: UploadRequestOptions): XMLHttpRequest | Promise<unknown> => {
  if (dataFinal.value.httpRequest) {
    dataFinal.value.httpRequest(options).then((res) => {
